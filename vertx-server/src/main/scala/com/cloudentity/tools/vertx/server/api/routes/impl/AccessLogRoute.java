...
public class AccessLogRoute extends RouteVerticle {
  private LoggingWithTracing log;
  private final Marker accessMarker = MarkerFactory.getMarker("ACCESS");
  ...
  protected void initService() {
    ...
    Optional<String> logger = Optional.ofNullable(getConfig()).flatMap(c -> Optional.ofNullable(c.getString("logger")));
    if (logger.isPresent()) {
      log = LoggingWithTracing.getLogger(logger.get());
    } else {
      log = LoggingWithTracing.getLogger(AccessLogRoute.class);
    }
  }
}
