...
public class ServiceClientFactory {
  ...
  public static <T> T makeWithTracing(EventBus bus, TracingManager tracing, Class<T> clazz, Optional<String> addressPrefixOpt, DeliveryOptions opts) {
    ...
    Map<Method, Function<Object[], Future<Object>>> methodHandlers = buildFutureMethodHandlers(bus, tracing, vertxEndpoints, opts);
    Map<Method, Consumer<Object[]>> methodVoidHandlers = buildVoidMethodHandlers(bus, vertxEndpoints, opts);
  ...
  public static <T> T makeWithTracing(EventBus bus, TracingManager tracing, Class<T> clazz, Optional<String> addressPrefixOpt) {
    return makeWithTracing(bus, tracing, clazz, addressPrefixOpt, new DeliveryOptions().setSendTimeout(VERTX_SERVICE_CLIENT_TIMEOUT));
  }

  public static <T> T makeWithTracing(EventBus bus, TracingManager tracing, Class<T> clazz) {
    return makeWithTracing(bus, tracing, clazz, Optional.empty(), new DeliveryOptions().setSendTimeout(VERTX_SERVICE_CLIENT_TIMEOUT));
  }

  public static <T> T make(EventBus bus, Class<T> clazz, Optional<String> addressPrefixOpt, DeliveryOptions opts) {
    return makeWithTracing(bus, JaegerTracing.noTracing, clazz, addressPrefixOpt, opts);
  }

  public static <T> T make(EventBus bus, Class<T> clazz, Optional<String> addressPrefixOpt) {
    return make(bus, clazz, addressPrefixOpt, new DeliveryOptions().setSendTimeout(VERTX_SERVICE_CLIENT_TIMEOUT));
  }

  public static <T> T make(EventBus bus, Class<T> clazz) {
    return make(bus, clazz, Optional.empty(), new DeliveryOptions().setSendTimeout(VERTX_SERVICE_CLIENT_TIMEOUT));
  }
  ...
  public static void assertAllReturnFutureOrVoid(Class clazz, List<VertxEndpointInterface> methods) {
